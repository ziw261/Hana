...
namespace Hana
{
	...
	Hana::Ref<Hana::Shader> ShaderLibrary::Load(const std::string& filepath)
	{
	...
	Hana::Ref<Hana::Shader> ShaderLibrary::Load(const std::string& name, const std::string& filepath)
	{
		auto shader = Shader::Create(filepath);
		Add(name, shader);
		return shader;
	...
	Hana::Ref<Hana::Shader> ShaderLibrary::Get(const std::string& name)
	{
